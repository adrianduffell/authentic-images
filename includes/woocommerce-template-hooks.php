...
function init_woocommerce_template_hooks(): void {
	add_action( 'woocommerce_product_meta_start', 'AuthenticImages\display_authentic_image_notice_hook', 1 );
}
...
/**
 * Display the authentic image notice in the product meta area on single product pages (classic themes only).
 *
 * Fired by `woocommerce_product_meta_start`.
 *
 * @internal WordPress action hook
 */
function display_authentic_image_notice_hook(): void {
	$product = wc_get_product( get_the_ID() );
